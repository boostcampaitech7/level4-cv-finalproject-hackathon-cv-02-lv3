...
# --------------------- #
    ...
    for index, row in X_train.iterrows():
        def objective_function(Lattitude, Longtitude, BuildingArea):
            ...
            return prediction + BuildingArea # 개별 행에 대한 예측값 반환

        # Bayesian Optimization 실행
        optimizer = BayesianOptimization(
            f=objective_function,
            pbounds={
                "Lattitude": (-38.18255, -37.40853), 
                "Longtitude": (144.43181, 145.5264),
                "BuildingArea": (X_simulation['BuildingArea'].min(), X_simulation['BuildingArea'].max())},
            random_state=42,
        )
    ...
    csv_filename = "optimized_solutions_add_BuildingArea.csv"

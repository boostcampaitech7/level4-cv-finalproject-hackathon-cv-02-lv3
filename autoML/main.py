...
def main(task_type='regression', n_population=30, n_generation=5, n_parent=5, prob_mutations=[0.2, 0.5], use_joblib=True, n_jobs=-1, use_kfold=True, kfold=5, timeout=30, seed=42):
    ...
    # with open(os.path.join(py_dir_path, "autoML_attrition.pkl"), "wb") as file:
    #     cloudpickle.dump(autoML, file)
    # print(f"Model saved in '{os.path.join(py_dir_path, 'autoML_attrition.pkl')}'.")
...
if __name__ == '__main__':
    ...
    # main(task_type='classification', prob_mutations=[0.2, -1]) # hyperparameter mutation X

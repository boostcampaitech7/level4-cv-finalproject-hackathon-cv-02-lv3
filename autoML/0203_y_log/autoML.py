...
class AutoML:
    ...
    """
    def __init__(self, n_population=20, n_generation=50, n_parent=5, prob_mutations=[0.2, 0.5], use_joblib=False, n_jobs=-1, y_scale_log=True):
        ...
        self.y_scale_log = y_scale_log

        dicts = {'n_population': n_population, 'n_generation': n_generation, 'n_parent': n_parent,
                 'prob_mutations': prob_mutations, 'use_joblib': use_joblib, 'n_jobs': n_jobs, 'y_scale_log': y_scale_log}
    ...
    def fit_structure(self, structure, timeout=30, order=0):
        ...
        try:
            for i in range(len(self.X_trains)):
                ...
                if self.y_scale_log:
                    train_metric = evaluate_regression(np.power(10, y_train), np.power(10, y_train_pred))
                    valid_metric = evaluate_regression(np.power(10, y_valid), np.power(10, y_valid_pred))
                else:
